...
업데이트: **${currentTime}**`;
        ...
        for (const channel of deepChannels) {
            ...
            try {
                ...
                // 현재 채널의 활성화된 심층 제보 가져오기
                const activeReports = await getActiveDeepReports(channel.deep_ch_id, currentTimestamp);
                console.log(`${discordChannel.name} 채널의 심층 제보 ${activeReports.length}개 가져오기 성공`);
                
                // 만료 및 오제보 처리의 개수 확인
                const expiredReports = activeReports.filter(report => report.is_expired);
                const errorReports = activeReports.filter(report => report.is_error === 'Y');
                
                console.log(`${discordChannel.name} 채널: 만료 ${expiredReports.length}개, 오제보 ${errorReports.length}개`);
                ...
                // 2. 버튼 상태 업데이트 (코드가 안정적으로 작동하는 것을 확인한 후 추가 기능 구현)
                try {
                    if (activeReports.length > 0) {
                        await updateReportButtons(discordChannel, activeReports);
                        console.log(`${discordChannel.name} 채널의 신고 버튼 검색 완료`);
                    }
                } catch (buttonError) {
                    console.error(`버튼 상태 업데이트 실패 (${discordChannel.name}):`, buttonError.message);
                }
        ...
        console.error('심층 제보 업데이트 오류:', error);
    }
}
